...
plt.text(0.7, 0.105, 'leakiness', color='w', backgroundcolor='b', fontsize=16,
        transform=ax.transAxes)
plt.hlines(0.565, 1E-7, 1.50217E-5, color='c', alpha=0.45, linewidth=5)
plt.vlines(1.50217E-5, 0, 0.565, color='c', alpha=0.45, linewidth=5)
plt.hlines(np.min(fc), 1E-7, 1E-3, color='b', linewidth=5, alpha=0.45)
plt.hlines(np.max(fc), 1E-7, 1E-3, color='g', linewidth=5, alpha=0.45)
plt.plot(1.50127E-5, 0.565,'o',color='c', alpha=0.85, markersize=12)
plt.text(0.05, 0.46, r'$EC_{50}$', color='w', backgroundcolor='c', fontsize=16,
        transform=ax.transAxes)
plt.plot(iptg_range, fc, 'r-', linewidth=2.5)
...
color='w', transform=ax.transAxes, fontsize=16)
plt.vlines(1.5E-3, np.min(fc),np.max(fc), color=p, linewidth=5)
plt.hlines(np.max(fc), 1.2E-3, 1.55E-3, color=p, linewidth=5)
plt.hlines(np.min(fc), 1.2E-3, 1.55E-3, color=p, linewidth=5)
plt.text(0.58E-3, 0.57, 'dynamic range', color='w', backgroundcolor=p,
        fontsize=16)

...
# Create legends
# List containing one empty rectangle
empty = [Rectangle((0, 0), 1, 1, fc="w", fill=False,
                   edgecolor='none', linewidth=0)]
ims = []
for m in markers:
    for i in range(len(Reps)):
        ims.append(mlines.Line2D([], [], marker=m, markerfacecolor='white',
                                 markeredgecolor=colors[i], markeredgewidth=1, markersize=5,  linestyle='None'))
legend_handles = empty * 8 + ims[:6] + empty + ims[6:12] + empty + ims[12:]

rep_labels = ['rep./cell', '1740', '1220', '260', '124', '60', '22']
op_labels = [['O1'], ['O2'], ['O3']]
empty_label = ['']
legend_labels = rep_labels + op_labels[0] + empty_label * 6 + \
    op_labels[1] + empty_label * 6 + op_labels[2] + empty_label * 6
ax3.legend(legend_handles, legend_labels, ncol=4, bbox_to_anchor=(
    1.2, 1.1), handletextpad=-1.3, fontsize=15, columnspacing=1, labelspacing=0.2, borderpad=0.3)
...
for ax in (ax1, ax2, ax3):
    ...
    ax.text(-20.5, 1.02, labels_dict[ax]['plotlabel'], fontsize=12)
mwc.scale_plot(fig, 'two_row')
plt.tight_layout()
plt.savefig('../../figures/SI_figs/figS26.pdf', bbox_inches='tight')

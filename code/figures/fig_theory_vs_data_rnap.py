...
# Define the variables for the RNAP
rnap = 3000 # RNAP / cell
epsilon_p = -5.35 # kBT
...
for i, op in enumerate(operators):
    ...
    for j, rbs in enumerate(df.rbs.unique()):
        ...
        ax[i].plot(IPTG, mwc.fold_change_log_rnap(IPTG * 1E6, 
            ea=ea, ei=ei, epsilon=4.5,
            R=df[(df.rbs == rbs)].repressors.unique(),
            epsilon_r=energies[op], P=rnap, epsilon_p=epsilon_p),
            color=colors[j])
        # plot 95% HPD region using the variability in the MWC parameters
        cred_region = mwc.mcmc_cred_region_rnap(IPTG * 1E6, 
...
plt.tight_layout()
plt.savefig(output + '/fig_theory_vs_data_O2_RBS1027_fit_RNAP.pdf')

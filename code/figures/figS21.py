    ...
    for j, rbs in enumerate(df.rbs.unique()):
        ...
        if rbs in data.rbs.unique():
            ...
            ax[0].plot(IPTG, mwc.fold_change_log(IPTG * 1E6,
                                                 ea=ea, ei=ei, epsilon=4.5, R=np.array(df[(df.rbs == rbs)].repressors.unique()), epsilon_r=energies[op]),
...
ax[0].set_title(r'$\Delta\varepsilon_{RA} = %s\,k_BT$' % energies[op], y=1.02,
                backgroundcolor='#FFEDCE')

ax[1].set_title(r'$\Delta\varepsilon_{RA} = %s\,k_BT$' % np.round(
    map_param[op], 1), y=1.02, backgroundcolor='#FFEDCE')


main_legend = ax[0].legend(loc='upper left', title='repressors / cell')


plt.figtext(0.0, .9, '(A)', fontsize=12)
plt.figtext(0.50, .9, '(B)', fontsize=12)
mwc.scale_plot(fig, 'one_row')
plt.tight_layout()
plt.savefig('../../figures/SI_figs/figS21.pdf', bbox_inches='tight')

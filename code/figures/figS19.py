...
for i, op in enumerate(operators):
    ...
    for j, rbs in enumerate(df.rbs.unique()):
        ...
        if op != 'Oid':
            ...
            fc_mean = data[data.rbs == rbs].groupby(
                'IPTG_uM').fold_change_A.mean()
    ...
    # ax[i].text(0.8, 0.09, r'{0}'.format(op), transform=ax[i].transAxes,
            #    fontsize=13)
    ax[i].set_title(r'%s $\Delta\varepsilon_{RA} = %s\,k_BT$' % (op, np.round(
        map_param[op], 1)),
        transform=ax[i].transAxes, backgroundcolor='#FFEDCE', y=1.08)
...
plt.figtext(0.0, .97, '(A)', fontsize=12)
plt.figtext(0.50, .97, '(B)', fontsize=12)
plt.figtext(0.0, .47, '(C)', fontsize=12)
plt.figtext(0.50, .47, '(D)', fontsize=12)
mwc.scale_plot(fig, 'two_row')
plt.tight_layout()
plt.savefig('../../figures/SI_figs/figS19.pdf', bbox_inches='tight')

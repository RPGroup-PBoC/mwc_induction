...
# Set the sclae and labels.
    ...
    ax[i].set_xlabel('IPTG (M)', fontsize=13)
    ax[i].set_ylabel('fold-change', fontsize=13)
...
ax[3].hlines(ka_paper, 0.8, len(repressors) + .2, linestyle='--', color='k',zorder=1, alpha=0.6)
ax[4].hlines(ki_paper, 0.8, len(repressors) + .2, linestyle='--', color='k',zorder=1, alpha=.6)
ax[3].fill_between(np.linspace(0, i, 1000), sel_dat['Ka_low'].values[0], sel_dat['Ka_high'].values[0], color='k', alpha=0.4)
...
ax[3].set_ylabel('$K_A\,\,(\mu\mathrm{M})$', fontsize=13)
ax[4].set_ylabel('$K_I\,\,(\mu\mathrm{M})$', fontsize=13)
"""

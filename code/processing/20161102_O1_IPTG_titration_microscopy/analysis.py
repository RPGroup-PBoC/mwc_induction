...
IPTG_RANGE = [0, 0.1, 5, 10, 25, 50, 75, 100, 250, 500, 1000, 50000]
# Load the data files.
...
plt.savefig('output/' + str(DATE) + '_' + OPERATOR +
            '_intensity_distributions.pdf', bbox_inches='tight')
...
plt.tight_layout()
plt.savefig('output/' + str(DATE) + '_' + OPERATOR +
            '_IPTG_mean_fluoresence.pdf', bbox_inches='tight')


# Plot the fold change vs the prediction.
epa = -np.log(141E-6)
epi = -np.log(0.56E-6)
epr = -15.8  # In units of kBT
iptg = np.logspace(-9, -1, 1000)
...
plt.savefig('output/' + str(DATE) + '_' + OPERATOR + '_IPTG_titration.pdf',
            bbox_inches='tight')

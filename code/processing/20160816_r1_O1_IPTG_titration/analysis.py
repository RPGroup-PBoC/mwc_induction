...
df = pd.read_csv('output/' + str(date) + '_' + run + \
...
plt.tight_layout()
plt.savefig('output/O1_IPTG_titration_data.png')

#=============================================================================== 

# plot the curve for the 0 IPTG cultures
repressor_array = np.logspace(0, 3, 200)
fc_theory = 1 / (1 + 2 * repressor_array / 5E6 * np.exp(15.3))
...
plt.ylabel('fold-change')
plt.tight_layout()
plt.savefig('output/O1_lacI_titration_ctrl.png')

...
# write
df.to_csv('output/' + str(date) + '_' + run +\
        '_O1_IPTG_titration_MACSQuant.csv', index=False)
#=============================================================================== 
# Add the comments to the header of the data file
filenames = ['./comments.txt', 'output/' + str(date) + '_' + run + \
             '_O1_IPTG_titration_MACSQuant.csv']
with open('../../../data/' + str(date) + '_' + run + \
          '_O1_IPTG_titration_MACSQuant.csv', 'w') as output:

...
for i, c in enumerate(concentrations):
    for j, strain in enumerate(rbs):
        # find the file
        try:
            r_file = glob.glob(datadir + str(date) + '_r2' + '*' + operator + '_' + \
...
# write
df.to_csv('output/' + str(date) + '_r2_O2_IPTG_titration_MACSQuant.csv', index=False)
#=============================================================================== 
# Add the comments to the header of the data file
filenames = ['./comments.txt', 'output/' + str(date) + '_r2_O2_IPTG_titration_MACSQuant.csv']
with open('../../../data/' + str(date) + '_r2_O2_IPTG_titration_MACSQuant.csv', 'w') as output:

...
plt.legend(loc='lower left')
plt.tight_layout()
plt.savefig('output/lacI_titration_FITC_H.png')

# Now for the ratio of the FITC-H and the FSC-A channels
plt.figure(figsize=(7, 7))
for i, o in enumerate(operators):
    fold_change_theor = 1 / (1 + 2 * repressor_array / 5E6 \
            * np.exp(-epsilon_array[i]))
...
plt.xlabel('repressor copy number')
plt.ylabel('fold-change')
plt.title('FITC-A / FSC-A')
plt.xlim(left=10)
plt.legend(loc='lower left')
plt.tight_layout()
plt.savefig('output/lacI_titration_FITC_r.png')

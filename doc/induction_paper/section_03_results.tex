...
we can now \textit{predict} expression for our other strains without any further fitting. Taking this approach, we chose to fit these parameters using our simple repression strain with $R=130$ and an O2 operator. The measured fold-changes in expression are shown in \fref[fig_result1] as a function of inducer concentration. Plotted along with our data is \eref[eq7] using our fitted values for $K_I$, $K_A$, and $\Delta\varepsilon_{IA}$.
...
\begin{figure}[h]
	...
	\caption{{\bf Fold-change measurements and fit from an IPTG titration using a simple repression construct containing 130 repressors per cell and O2 operator.} Fold
		...
		and $\Delta\varepsilon_{IA}$. Values for LacI copy number and operator binding energy
		...
		$\Delta\varepsilon_{IA}$. \talComment{We should either explicitly state the parameter values here or have a table with them in the text (I vote for the former)}} \label{fig_result1}
\end{figure}
...
\begin{figure}[h]
	...
	\caption{{\bf Our model of induction can be used to predict fold-change in expression as we tune operator binding energy and repressor copy number. \talComment{Caption heading should not be 1st person. Can change it to something like: ``Theoretical predictions versus experimental measurements for fold-change using different operator binding energies and repressor copy numbers''}} \tiny{\stephanieComment{Is this
...
In the above approach we estimated $K_I$, $K_A$, and $\Delta\varepsilon_{IA}$
...
\begin{figure}[h]
	...
	\caption{{\bf Fold-change data from 24 different strains were plotted as a function
		...
		$\Delta\varepsilon_{IA}$ from strain RBS1027 with an O2 operator.

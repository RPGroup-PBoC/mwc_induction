...
\robComment{I would emphasize what the cell cares about is the Bohr parameter.
	...
	emphasize. \talComment{Building off this point, can add something like:
		...
		tuning different biological parameters \cite{Natarajan2016}. Indeed, in Appendix
		\ref{AppendixSloppiness}, we discuss how the $K_A$, $K_I$, and $\Delta
		\varepsilon_{AI}$ parameters are not completely independent.'' A really nice
		quote in \cite{Natarajan2016} is that there is a ``many-to-one mapping of
		genotype to phenotype,''}} with a single set of thermodynamic parameters.

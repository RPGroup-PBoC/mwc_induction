...
the form of signaling pathways \talComment{maybe finish this sentence with two
	examples?}. One of the most pervasive categories of cell signaling is allosteric
regulation, \talComment{where ligand binding induces a conformational change in
	transcription factors, which thereby affects gene expression} \sout{which
...
inflection region, and the dynamic range \talComment{I really dislike this
	sentence because: (1) We have not defined what any of these terms mean; (2) We
	do not discuss any of these properties in our paper. I would prefer something
	like our previous version: ``At this level, fine details of a system - the
	steepness of the response, the limits of minimum and maximum response, etc. -
	can now be resolved to go beyond a general sigmoidal shapes.''}. The inputs to
...
proper context. \talComment{The last two sentences seem weak, and we should
	definitely remove the double use of ``strong''}
...
\talComment{When do we discuss cooperativity???} between transcription factor
subunits. \stephanieComment{At this point we don't actually discuss variations
	to the allosteric properties of the transcription factor, but I like the idea of
...
\talComment{awk}. We then assess the predictive nature of our model with the
following question: once we have determined the three parameters \talComment{We
	should not say three parameters unless we explicitly state what they are} that

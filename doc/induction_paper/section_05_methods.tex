...
binding site mutants were cloned into a pZS3\*5 plasmid backbone. Expression of
...
tetramer in \textit{E. coli} \griffinComment{find a citation} in which a single
...
allosterically independent \griffinComment{Include a reference here to the
dimer data}. This means that a single LacI molecule can be treated as two
...
\begin{figure}[h]
\centering
	\includegraphics[scale=0.5]{supplementary_figures/SI_plate_arrangement.pdf}
...
\talComment{awk} which display a high degree of linear correlation  between
...
\cite{Aghaeepour2013, Lo2008}. In these methods \talComment{it is a bit hard to
understand that ``these methods'' does not refer to our project}, however, the
...
is found \talComment{This is confusing}. This  was performed by fitting a
bivariate Gaussian distribution and restricting the data used for calculation
as those that reside within the 40th percentile. \griffinComment{Let's make
sure that the Jupyter notebook for this gating is crystal clear and well
rendered. I feel that this is going to be a serious sticking point for any fc
expert reviewers.} This procedure is  described in more detail in the
...
\talComment{this could be a hyperlink}.
...
energy $\Delta\epsilon_r$, and the IPTG concentration $C$ \talComment{Why
capitalize $c$ here?}). Using these assumptions, we can state our likelihood
...
C^i))^2}{2\sigma^2}\right], \end{align} where \talComment{link and punctuation,
both here and below} $\sigma$ is the variance associated with the Gaussian
distributed error of the MAP, $\mathrm{fc}$ is the the fold-change expression
\talComment{why did you use $\mathrm{fc_{exp}}^i$ and not
$\mathrm{fc_{exp}^i}$? Note that you used subsuperscripts in
$\Delta\epsilon_r^i$} derived previously \griffinComment{reference the main
text equation here once we converge on notation}, $n$ is the number of

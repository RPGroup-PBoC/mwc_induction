...
\griffinComment{I agree with Stephanie. Furthermore, we really need to determine whether we are telling the two or three parameter story. We should hammer this down ASAP. \talComment{I think we should stick with the full 3 parameter model, and we can discuss the 2 parameter stuff in the SI}}
...
state of repressor and RNA polymerase occupancy \manuelComment{occupancy of what? we should mention the promoter} is proportional to its Boltzmann
...
represents the energy difference between RNAP bound to the Lac operator \manuelComment{the RNAP does not bind to the lac operator} or bound
...
%	\centering \includegraphics[scale=\globalScalePlots]{states_weights_allosteric_independence.eps}
	\caption{{\bf States and weights for simple repression.} \stephanieComment{We should move the operator so that the repressor isn't covering up the arrow--it's a confusing visual. \talComment{Griffin, your eps figure won't show up. Any reason why not to make it a pdf? Also, I don't have your fonts}} Both RNAP (light blue)
		...
		weights comes from the ability of either repressor dimer to bind to DNA. \talComment{Need to refer somewhere around here to the Appendix section with our dimer data}}
...
probability $p_{\text{bound}}$ that RNAP is bound to the Lac operator \manuelComment{again this is not true} which is
...
can be empty or occupied), using the difference in energy $\varepsilon$ \manuelComment{if it is an energy difference I think it should be $\Delta \varepsilon$} between a
